...
// какеи комнаты есть на сервере
const rooms = ['general', 'random', 'practice'];

io.on('connection', socket => {
  // функция возвращает массив имен, кто находится в комнате room
  ...
  socket.on('adduser', function (username) {
    ...
    socket.json.emit('updaterooms', { rooms, current: defaultRoom });
    // сообщаем пользователю список пользователей в комнате
    socket.emit('updateusers', updateUsers(defaultRoom));
    // сообщаем всем в комнате, что список пользоватлей поменялся
    socket.broadcast
      .to(defaultRoom)
      .emit('updateusers', updateUsers(defaultRoom));
  });
  ...
  socket.on('switchroom', function (newroom) {
    ...
    socket.emit('updateusers', updateUsers(newroom));
    // в новой комнате формируем новый список пользователей комнаты другим
    // пользователям
    socket.broadcast.to(newroom).emit('updateusers', updateUsers(newroom));
  ...
  // обрабатываем событие когда пользователь ушел
  socket.on('disconnect', function (data) {
    ...
    socket.broadcast.to(room).emit('updateusers', updateUsers(room));
  });
});
